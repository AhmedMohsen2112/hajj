...
//        handleTruncate();
    ...
    var handleReport2 = function () {
        $('.btn-report').on('click', function () {
            var data = $("#filter-reports").serializeArray();


            var url = config.admin_url + "/pilgrims";
            var params = {};
            ...
            Pilgrims_grid.on('preXhr.dt', function (e, settings, data) {
                data
            })
            Pilgrims_grid.api().ajax.url(config.admin_url + "/pilgrims").load();
            return false;
        })
    }
        ...
        Pilgrims_grid = $('.dataTable').dataTable({
            ...
            "ajax": {
                ...
                data: {
                            ...
                            pilgrim_class: filter.pilgrim_class,
                            code: filter.code,
                            reservation_no: filter.reservation_no
                },
            },
            "columns": [
...
//                        $('#importPilgrims').modal('hide');
    ...
    return {
                ...
                buttons: {
                    danger: {
                        ...
                        callback: function (ele) {
                            ...
                            setTimeout(function () {
                                $.ajax({
                                    url: config.admin_url + "/pilgrims/truncate",
                                    data: {_token: $('input[name="_token"]').val()},
                                    success: function (data) {
                                        ele.target.setAttribute("disabled", false);
                                        ele.target.innerHTML = lang.apply;
                                        console.log(data);
                                        My.toast(data.message);
                                        Pilgrims_grid.api().ajax.reload();
                                        truncate.modal('hide')
        ...
        generateQr: function () {
            $('#importPilgrims').modal('hide');
            $('#importPilgrims').on('hidden.bs.modal', function () {
                 qr = bootbox.dialog({
                    closeButton: false,
                    animate: false,
                    message: '<p class="text-center bold">' + lang.qr_generate_message + '</p>',
                    buttons: {
                        danger: {
                            label: lang.apply,
                            className: "btn-truncate",
