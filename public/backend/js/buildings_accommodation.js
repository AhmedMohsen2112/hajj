...
var BuildingsAccommodation = function () {
    ...
    var init = function () {
        ...
        if (config.action != 'index') {
            ...
            handleReport();
            handleCheckAll();
            handleChangeBuildingFilter();
            handleChangeFloorFilter();
        }
    ...
    var handleChangeBuildingFilter = function (ele, suite) {
        $(document).on('change', 'select[name="building"]', function () {
            ...
            if (building && building != '') {
                $.get('' + config.admin_url + '/ajax/getFloors?building=' + building, function (data) {
    ...
    var handleChangeFloorFilter = function (ele, suite) {
        $(document).on('change', 'select[name="floor"]', function () {
            var floor = $(this).val();
            var html = '<option value="">' + lang.choose + '</option>';
            if (building && building != '') {
                $.get('' + config.admin_url + '/ajax/getRooms?floor=' + floor, function (data) {
                    if (data.data.length != 0)
                    {
                        $.each(data.data, function (index, Obj) {
                            html += '<option value="' + Obj.id + '">' + Obj.number + '</option>';
                ...
                buttons: {
                    danger: {
                        ...
                        callback: function (ele) {
                            ...
                            setTimeout(function () {
                                $.ajax({
                                    url: config.admin_url + "/buildings_accommodation/truncate",
