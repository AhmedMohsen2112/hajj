...
class SuiteAccommodation extends MyModel {
    ...
    private static function handleWhere($pilgrims, $where_array) {
        ...
        if (isset($where_array['filter'])) {
            ...
            if (isset($where_array['filter']['suite'])) {
                $pilgrims->where("suites.id", $where_array['filter']['suite']);
            }
            if (isset($where_array['filter']['lounge'])) {
                $pilgrims->where("lounges.id", $where_array['filter']['lounge']);
            }
