...
class BuildingAccommodation extends MyModel {
    ...
    public static $types = [
        'building_floor_room_1',
        'building_floor_room_2',
        'building_room'
    ];
    ...
    public static function gePilgrimsWithAccommodation($where_array = array(), $id = false) {
        ...
        if ($id) {
            ...
            if (isset($where_array['filter'])) {
                ...
                if (isset($where_array['filter']['per_page']) && $where_array['filter']['per_page'] == 'all') {
    ...
    private static function handleWhere($pilgrims, $where_array) {
        ...
        if (isset($where_array['filter'])) {
            ...
            if (isset($where_array['filter']['building'])) {
                $pilgrims->where("buildings.id", $where_array['filter']['building']);
            }
            if (isset($where_array['filter']['floor'])) {
                $pilgrims->where("buildings_floors.id", $where_array['filter']['floor']);
            }
            if (isset($where_array['filter']['room'])) {
                $pilgrims->where("buildings_floors_rooms.id", $where_array['filter']['room']);
            }
