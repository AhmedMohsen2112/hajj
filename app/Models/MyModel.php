...
class MyModel extends Model {
    ...
    public static $accommodation_phrases = [
        ...
        'بإمكانك معرفة تفاصيل سكنك بمنى وإمكانية إصدار بطاقة إلكترونية ',
        'عبر تطبيق شركة العلياني لحجاج الداخل .',
    ];
    public static $buses_accommodation_phrases = [
        'supervisors' => [
            'اخي المرشد',
            'بإمكانك الان معرفة الحجاج التابعين لك ومعرفة عددهم وكل تفاصيل الحاج ',
    ...
    protected static function deleteUploaded($path, $old_image) {
        if (in_array($old_image, ['default.png', 'bus.png', 'male.png', 'female.png'])) {
            return;
        }
        if (isset(static::$sizes) && !empty(static::$sizes)) {
            $files = array();
