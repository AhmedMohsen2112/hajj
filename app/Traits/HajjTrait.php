...
trait HajjTrait {
    ...
    protected function _import_csv_hajj($table, $csv, $extra_params) {
        ...
        $now=Carbon::now();
        $query = sprintf("LOAD DATA local INFILE '%s' INTO TABLE $table CHARACTER SET utf8 FIELDS TERMINATED BY ';' OPTIONALLY ENCLOSED BY '\"' ESCAPED BY '\"' LINES TERMINATED BY '\\n' IGNORE 1 LINES (`ssn`, `name`, `nationality`, `mobile`,`reservation_no`,`code`,`location_id`,`pilgrim_class_id`,`gender`,`image`,`created_at`,`updated_at` ) SET gender=TRIM(BOTH '\\r' FROM gender),updated_at='$now',created_at='$now'", addslashes($path));
    ...
    public function getNextSerialNumber($id, $prefix) {
        ...
        return $prefix . sprintf('%04d', intval($id));
    }
