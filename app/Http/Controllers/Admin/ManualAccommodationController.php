...
class ManualAccommodationController extends BackendController {
    ...
    private function getPilgrimWithNoAccommodation($where_array = array()) {
        $pilgrims = Pilgrim::leftJoin('suites_accommodation', 'pilgrims.id', '=', 'suites_accommodation.pilgrim_id')
                ...
                ->where(function ($query) use($where_array) {
                    $query->where('pilgrims.ssn',$where_array['pilgrim_code']);
                    $query->orWhere('pilgrims.code', $where_array['pilgrim_code']);
                })
    ...
    public function getFloors(Request $request) {
        $pilgrim = Pilgrim::where('code', $request->input('pilgrim_code'))->orWhere('ssn', $request->input('pilgrim_code'))->first();
        $floors = [];
        if ($pilgrim) {
