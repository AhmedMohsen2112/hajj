...
class BasicController extends ApiController {
    ...
    public function getSettings() {
        try {
            ...
            $cash_message= json_decode($settings['cash_message']->value);
            $about_text = json_decode($settings['about_text']->value);
            $supervisors = json_decode($settings['supervisors']->value);
            unset($settings['supervisors']);
            unset($settings['about_text']);
            unset($settings['cash_message']);
            $settings['social_media'] = json_decode($settings['social_media']->value);
            $settings['about_text'] = $about_text->{$this->lang_code};
            $settings['cash_message'] = $cash_message->{$this->lang_code};
...
//            $settings['info'] = SettingTranslation::where('locale', $this->lang_code)->first();
    ...
    public function getNotifications(Request $request) {
        ...
        try {
            ...
            $admin_notifications->where('admin_notifications.created_at', '>', $device->created_at);
            if ($type == 0) {
