    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <div class="row">
                ...
                <div class="form-group col-sm-4">
                    <label class="col-sm-3 inputbox utbox control-label">{{_lang('app.suite')}}</label>
                    <div class="col-sm-9 inputbox">
                        <select class="form-control" name="suite" id="suite">
                            <option value="">{{_lang('app.choose')}}</option>
                            @foreach($suites as $one)
                            <option {{ (isset($suite) && $suite==$one->id) ?'selected':''}} value="{{$one->id}}">{{$one->number}}</option>
                            @endforeach
                ...
                <div class="form-group col-sm-4">
                    <label class="col-sm-3 inputbox utbox control-label">{{_lang('app.lounge')}}</label>
                    <div class="col-sm-9 inputbox">
                        <select class="form-control" name="lounge" id="lounge">
                            <option value="">{{_lang('app.choose')}}</option>
...
<div class="table-toolbar">
    <div class="row">
        <div class="col-md-6">
            <div class="btn-group">
                <a class="btn green" href="{{ route('suites_accommodation.create') }}">{{ _lang('app.add_new')}}<i class="fa fa-plus"></i> </a>
            </div>
            <div class="btn-group">
                <a class="btn btn-warning" href="" onclick="SuitesAccommodation.truncate(); return false;">{{ _lang('app.emptying')}}</a>
            </div>
            <div class="btn-group">
                <form action="{{url('admin/suites_accommodation/download')}}" method="post">
                    {{ csrf_field() }}
                    ...
                    <input type="hidden" name="suite" value="{{ (isset($suite)) ?$suite:''}}">
                    <input type="hidden" name="lounge" value="{{ (isset($lounge)) ?$lounge:''}}">
